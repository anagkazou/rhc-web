...
export async function POST(request: NextRequest) {
  ...
  console.log(`[partner-submissions ${reqId}] received`, {
  ...
  if (!SUPABASE_URL || !SUPABASE_KEY) {
    console.error(`[partner-submissions ${reqId}] missing env vars`);
  ...
  try {
    ...
    try {
      ...
      console.error(`[partner-submissions ${reqId}] invalid JSON body`, parseErr);
      return NextResponse.json(
    ...
    console.log(`[partner-submissions ${reqId}] payload`, {
    ...
    if (!telegram_username || !email || !message) {
      console.warn(`[partner-submissions ${reqId}] missing required fields`);
      return NextResponse.json(
    ...
    if (
      ...
      console.warn(`[partner-submissions ${reqId}] non-string fields`);
      return NextResponse.json(
    ...
    if (!emailRegex.test(email)) {
      console.warn(`[partner-submissions ${reqId}] invalid email format`);
      return NextResponse.json(
    ...
    console.log(`[partner-submissions ${reqId}] inserting`, {
    ...
    if (error) {
      console.error(`[partner-submissions ${reqId}] supabase error`, {
        code: error.code,
    ...
    console.log(`[partner-submissions ${reqId}] success`, {
      durationMs: Date.now() - startedAt,
    });

    return NextResponse.json({ success: true }, { status: 201 });
  } catch (error) {
    console.error(`[partner-submissions ${reqId}] unexpected error`, {
